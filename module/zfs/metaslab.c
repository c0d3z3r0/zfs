...
metaslab_should_allocate(metaslab_t *msp, uint64_t asize, boolean_t try_hard)
{
	 ...
	 * try_hard and it's been less than zfs_metaslab_max_size_cache_sec seconds since
	 * the metaslab was unloaded.
	 */
	if (msp->ms_loaded ||
	    (msp->ms_max_size != 0 && !try_hard &&
	    gethrtime() < msp->ms_unload_time + SEC2NSEC(zfs_metaslab_max_size_cache_sec)))
